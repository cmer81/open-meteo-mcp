...
export const CellSelectionSchema = z
  ...
  .describe(
    'Grid cell to use near a coastline: prefer "land", prefer "sea", or the "nearest" cell.',
  );
...
export const TiltSchema = z
  ...
  .max(90)
  .describe(
    'Solar panel tilt in degrees (0 = flat, 90 = vertical); with `azimuth`, sets global_tilted_irradiance.',
  );
...
export const AzimuthSchema = z
  ...
  .max(180)
  .describe(
    'Solar panel azimuth in degrees (0 = south, -90 = east, 90 = west, ±180 = north); used with `tilt`.',
  );
...
export const HourlyTimestampSchema = z
  ...
  .describe('Timestamp YYYY-MM-DDTHH:MM; hourly-precision alternative to start_date/end_date.');

const PAST_DAYS_DESCRIPTION = 'Days before today to include, in addition to the forecast.';
const PAST_HOURS_DESCRIPTION = 'Hours before now to include; alternative to `past_days`.';
...
// Pressure-level variables are the full cross product of a few base variables
// and the 19 standard levels below. Listing each combination as an enum member
// published ~130 near-identical strings per tool (and twice per forecast tool,
// via `current`), which dominated the tools/list payload. A template literal
// accepts exactly the same set but publishes as a single regex `pattern`.
...
function pressureLevelVariableSchema<const T extends readonly [string, ...string[]]>(variables: T) {
  return z
    .templateLiteral([z.enum(variables), '_', z.enum(PRESSURE_LEVELS), 'hPa'])
    .describe(`Pressure-level variable <variable>_<level>hPa, e.g. "${variables[0]}_850hPa".`);
}

// A union's own error is a bare "Invalid input", which gives the caller nothing
// to act on when a variable name is misspelled.
const UNKNOWN_HOURLY_VARIABLE_MESSAGE =
  'Unknown variable: use one of the listed surface variables, or a pressure-level variable named <variable>_<level>hPa (e.g. "temperature_850hPa").';
...
export const HourlyVariablesSchema = z
  .array(
    z.union(
      ...
      { error: UNKNOWN_HOURLY_VARIABLE_MESSAGE },
    ),
  )
  .optional();
