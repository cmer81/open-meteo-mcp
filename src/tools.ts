  ...
  inputSchema: {
    ...
    properties: {
      ...
      models: {
        type: 'string',
        description:
          'Accepts a single model identifier (e.g., "best_match", "dwd_icon_global", "ncep_gfs_global"). For multi-model comparison, make one parallel tool call per model. Omit this parameter to let the API automatically select the best model for the location (recommended).',
      },
    },
...
export const WEATHER_MODEL_TOOLS: Tool[] = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        models: {
          type: 'string',
          description:
            'Accepts a single model identifier. Valid values: "ecmwf_ifs" (IFS HRES high-resolution), "ecmwf_ifs025" (IFS open-data 0.25°), "best_match". For multi-model comparison, make one parallel tool call per model. Omit to use the endpoint default.',
        },
      },
    },
  },
  ...
  },
  {
    name: 'metno_forecast',
    description:
      'Get weather forecast from Norwegian Meteorological Institute models. IMPORTANT: Specify exactly one Met.no model in the `models` parameter (e.g., "met_norway_nordic") — only one model per request is supported. For multi-model comparison, make one parallel tool call per model using the appropriate provider-specific tool.',
    inputSchema: WEATHER_FORECAST_TOOL.inputSchema,
  },
  {
    name: 'gem_forecast',
  ...
  inputSchema: {
    ...
    properties: {
      ...
      models: {
        type: 'string',
        description:
          'Accepts a single model identifier (e.g., "gfs_seamless", "icon_seamless_eps"). For multi-model comparison, make one parallel tool call per model.',
      },
      hourly: {
        type: 'array',
        items: {
